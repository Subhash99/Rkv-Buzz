...
public class EventsSection extends AppCompatActivity {
    ...
    DatabaseReference mDatabase = fd.getReference("Events");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                {
                    if (ContextCompat.checkSelfPermission(EventsSection.this, android.Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                        selectPdf();
                    } else
                        ActivityCompat.requestPermissions(EventsSection.this, new String[]{android.Manifest.permission.READ_EXTERNAL_STORAGE}, 9);
                }
            }
        });
    }
    ...
    private void uploadFile() {
        ...
        storageReference.child("EventsUploads").child(filename).putFile(pdfUri)
    ...
    private void work(String url) {
        ...
        Date todayDate = new Date();
        String thisDate = currentDate.format(todayDate);
        ex = new Events(PublicLink, LocalLink, Header, Desc, thisDate, url);
        mDatabase.child(id).setValue(ex);
        Notify nt = new Notify();
        nt.execute(Header, Desc);
        Toast.makeText(getApplicationContext(), "Successfull", Toast.LENGTH_SHORT).show();
        Intent i=new Intent(this,EventsEd.class );
        startActivity(i);
